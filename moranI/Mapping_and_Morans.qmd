...
The first step in a Moran's I analysis requires that we define "neighboring" polygons. This could refer to contiguous polygons, polygons within a certain distance, or even non-spatial relationships defined by social, political, or cultural “neighbors.”

Here, we'll adopt a contiguous neighbor definition. We'll accept any contiguous polygons that share at least one vertex; this is known as the "queen" case in the chess analogy and it is parameterized as `queen = TRUE` in the call to `poly2nb`. If we required that just *edges* be shared between polygons then we would set `queen = FALSE` (the *rook* case).
...
For each polygon in our spatial object, `nb` lists all neighboring polygons. For example, to see the neighbors (by ID number) for the first polygon in the spatial object, type:
...
These are the weights that each neighboring income value will be multiplied by before being summed. If a polygon has 5 neighbors, each neighbor will have a weight of 1/5 or 0.2. This weight will then be used to compute the mean neighbor value as follows: `0.2(neighbor1) + 0.2(neighbor2) + 0.2(neighbor3) + 0.2(neighbor4) + 0.2(neighbor5)`. This is equivalent to summing all five income values then dividing by 5.

### Step 3: Compute the (weighted) mean income of neighboring polygons (optional step)
...
Next, we'll have R compute the average neighbor income value for each polygon. These values are commonly referred to as **spatially lagged** values.
...
The null hypothesis states that the income values are randomly distributed across counties. There are two methods for testing this hypothesis: an analytical method and a Monte Carlo method. We'll explore both approaches in the following example.
...
The `moran.mc` function takes another parameter called `alternative =`. This parameter has three possible values: `"greater"` (the default), `"less"`, and `"two.sided"`. The choice will be dictated by the side of the distribution we want to compute the p-value for. If our observed Moran's I is to the right of the expected distribution, we will want to adopt the `"greater"` option which will focus on the upper tail of the distribution. If our observed value is to the left of the distribution, we will want to choose the `"less"` option to focus on the lower tail of the distribution. You can usually tell from the computed Moran's I value which tail you will want to emphasize by its sign. A general rule of thumb is to place emphasis on the lower tail if Moran's I value is negative, and to place emphasis on the upper tail if Moran's I value is positive. In the example, our Moran's I value of `r round(unlist(I),2)` is positive so we'll choose `"greater"` for the parameter.
...
The curve shows the null distribution of Moran I values we could expect if income values were randomly distributed across counties. Note that our observed statistic, `r round(spdep::moran.test(s$Income,lw)$estimate[1],3)`, falls way to the right of the distribution suggesting that the income values are clustered (a positive Moran's I value suggests clustering whereas a negative Moran's I value suggests dispersion).
...
# Compute the average neighboring homicide rate
lw <- nb2listw(nb, style="W", zero.policy=TRUE)
