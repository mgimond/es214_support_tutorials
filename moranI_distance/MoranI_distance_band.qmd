...
We will explore spatial autocorrelation as a function of distance bands. Instead of defining neighbors as contiguous polygons, we will define neighbors based on distances to polygon centers. A neighboring polygon will be defined as any polygon whose centroid lies within a specified distance of the centroid of the polygon of interest. This will therefore require that we compute the centroid of each census tract polygon using `st_point_on_surface`. The centroids' coordinate pairs will then be extracted using the `st_coordinates` function.
...
Next, we will define a neighbor as any polygon whose centroid lies within 5 km (or 5,000 map units in this example) of each polygon.

```{r}
s.dist  <-  dnearneigh(s.coord, 0, 5000)  
```

The warning simply indicates that many polygons do not share neighbors within the specified distance band and therefore form disconnected groups.

The `dnearneigh` function takes three parameters: the coordinate values `s.coord`, the inner radius of the annulus band (first number in the function), and the radius for the outer annulus band (second number in the function). In our example, the inner annulus radius is `0` and the outer annulus radius is `5000` which implies that all polygon centers up to 5 km are considered neighbors (i.e. the current example creates a full circle search radius).
...
Note that large polygons may have fewer neighbors because the distances between their centroids and the centroids of adjacent polygons tend to be greater. For example:
